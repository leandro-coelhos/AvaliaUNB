...
class Disciplina(db.Model):
    ...
    fk_codigo_departamento = db.Column('fk_Cod_Dep',
                                       db.String(10),
                                       db.ForeignKey('Dep.Cod_Dep'),
                                       nullable=False)
...
class TipoUsuario(db.Model):
    ...
    codigo_tipo_usuario = db.Column('Cod_Tp_Usr',
                                    db.SmallInteger,
                                    primary_key=True)
...
class Usuario(db.Model):
    ...
    numero_identificacao_usuario = db.Column('Num_Idf_Usr',
                                             db.Integer,
                                             primary_key=True)
    ...
    fk_codigo_tipo_usuario = db.Column('fk_Cod_Tp_Usr',
                                       db.SmallInteger,
                                       db.ForeignKey('Tp_Usr.Cod_Tp_Usr'),
                                       nullable=False)
...
class PeriodoLetivo(db.Model):
    ...
    codigo_periodo = db.Column('Cod_Per',
                               db.Integer,
                               primary_key=True,
                               autoincrement=True)
    ...
    turmas = db.relationship('Turma',
                             backref='periodo',
                             lazy=True,
                             foreign_keys='Turma.fk_codigo_periodo')


class Turma(db.Model):
    __tablename__ = 'Tur'

    numero_identificacao_turma = db.Column('Num_Idf_Tur',
                                           db.SmallInteger,
                                           primary_key=True)
    fk_codigo_disciplina = db.Column('fk_Cod_Dis',
                                     db.String(10),
                                     db.ForeignKey('Dis.Cod_Dis'),
                                     nullable=False)
    fk_codigo_periodo = db.Column('fk_Cod_Per',
                                  db.String(10),
                                  db.ForeignKey('Per_Let.Cod_Per'),
                                  nullable=False)
    professor_codigo = db.Column('fk_Cod_Prof',
                                 db.SmallInteger,
                                 db.ForeignKey('Prof.Cod_Prof'),
                                 nullable=True)

    criterios_avaliacao = db.relationship('CriterioAvaliacaoTurma',
                                          backref='turma',
                                          lazy=True)
...
class TipoAvaliacao(db.Model):
    ...
    codigo_tipo_avaliacao = db.Column('Cod_Tp_Aval',
                                      db.SmallInteger,
                                      primary_key=True)
    nome_tipo_avaliacao = db.Column('Nom_Tp_Aval', db.String(25))

    criterios_avaliacao = db.relationship('CriterioAvaliacaoTurma',
                                          backref='tipo_avaliacao',
                                          lazy=True)
...
class CriterioAvaliacaoTurma(db.Model):
    ...
    numero_identificacao_avaliacao = db.Column('Num_Idf_Aval',
                                               db.Integer,
                                               primary_key=True)
    fk_numero_identificacao_turma = db.Column('fk_Num_Idf_Tur',
                                              db.SmallInteger,
                                              db.ForeignKey('Tur.Num_Idf_Tur'),
                                              nullable=False)
    fk_codigo_tipo_avaliacao = db.Column('fk_Cod_Tp_Aval',
                                         db.SmallInteger,
                                         db.ForeignKey('Tp_Aval.Cod_Tp_Aval'),
                                         nullable=False)

    documentos = db.relationship('DocumentoAvaliacao',
                                 backref='criterio_avaliacao',
                                 lazy=True)
...
class DocumentoAvaliacao(db.Model):
    ...
    numero_identificacao_documento = db.Column('Num_Idf_Doc',
                                               db.Integer,
                                               primary_key=True)
    ...
    fk_numero_identificacao_avaliacao = db.Column(
        'fk_Num_Idf_Aval',
        db.Integer,
        db.ForeignKey('Crit_Aval_Tur.Num_Idf_Aval'),
        nullable=False)
    # Campos adicionais para associar ao feedback específico
    fk_usuario_id = db.Column('fk_Usr_Id',
                              db.Integer,
                              db.ForeignKey('Usr.Num_Idf_Usr'),
                              nullable=True)
    fk_professor_id = db.Column('fk_Prof_Id',
                                db.SmallInteger,
                                db.ForeignKey('Prof.Cod_Prof'),
                                nullable=True)
    fk_turma_id = db.Column('fk_Tur_Id',
                            db.SmallInteger,
                            db.ForeignKey('Tur.Num_Idf_Tur'),
                            nullable=True)
...
class Feedback(db.Model):
    ...
    pfk_numero_identificacao_turma = db.Column(
        'pfk_Num_Idf_Tur',
        db.SmallInteger,
        db.ForeignKey('Tur.Num_Idf_Tur'),
        primary_key=True)
    pfk_codigo_professor = db.Column('pfk_Cod_Prof',
                                     db.SmallInteger,
                                     db.ForeignKey('Prof.Cod_Prof'),
                                     primary_key=True)
    pfk_numero_identificacao_usuario = db.Column(
        'pfk_Num_Idf_Usr',
        db.Integer,
        db.ForeignKey('Usr.Num_Idf_Usr'),
        primary_key=True)
